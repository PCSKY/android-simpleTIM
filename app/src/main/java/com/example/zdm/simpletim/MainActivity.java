...
import com.example.zdm.simpletim.adapter.PersonAdapter;
import com.example.zdm.simpletim.dao.PersonDAO;
import com.example.zdm.simpletim.model.Person;
import com.example.zdm.simpletim.utils.WordsNavigation;
...
public class MainActivity extends AppCompatActivity
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        //顶部工具栏(在app_bar_main.xml中)
        Toolbar toolbar = (Toolbar) findViewById(R.id.toolbar);
        setSupportActionBar(toolbar);

        //Android侧滑菜单DrawerLayout(activity_main.xml)
        ...
        //设置左侧菜单(在app_bar_main.xml中)
        NavigationView navigationView = (NavigationView) findViewById(R.id.nav_view);
        navigationView.setNavigationItemSelectedListener(this);

        //发表说说(在app_bar_main.xml中)(todo)
    ...
    public void onScroll(AbsListView view, int firstVisibleItem, int visibleItemCount, int totalItemCount) {
        ...
        word.setTouchIndex(list.get(firstVisibleItem).getHeaderChar());
    }
    ...
    private void updateListView(String words) {
        for (int i = 0; i < list.size(); i++) {
            String headerWord = list.get(i).getHeaderChar();
